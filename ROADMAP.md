...
- [x] **Session 37:** Scheduler `_load_config` import fix (scheduled recs crashed since Session 36); Kimi K2.6 found 404ing on NVIDIA's side; SESSION_LOG split (1–30 archived); hallucinated 850k fodder rec dismissed.
- [x] **Session 38:** NVIDIA provider health probe — `health_check()` on providers, `probe_all_providers()` registry helper (parallel, Anthropic assumed up with key), scheduler `provider_probe` job every 30 min + `POST /probe-providers` cached endpoint, Electron `providerHealth` Map + `db:getProviderHealth` IPC, Ask chips + Recommendations dropdown grey unhealthy models with "Temporarily unavailable" tooltip and auto-restore. Probe semantics: HTTP error = unhealthy (pulled endpoints 404 in 0.2s), timeout = healthy (DeepSeek cold-start 55–120s; reasoning models stall below ~2000 max_tokens).
- [x] **Session 38:** Context builder name-collision fix — `build_context(card_id=...)` pins the exact card for autonomous recs; fuzzy name matching had fed Bellingham Summer Stars the 2170h-old age of the legacy Bellingham TOTS seed (8 versions matched). Verified: each version now gets exactly its own context.
- [x] **Session 38:** Fodder hallucination guard — targets >10× the actual cheapest BIN are dropped before insert (Mistral turned "850" into 850,000); fodder prompt now states prices are raw coin values, never thousands.
- [x] **Session 39:** Concurrent-run dedup lock — `_rec_lock`/`run_recommendations_safe()` in scheduler; scheduled job and HTTP trigger share it; second caller skips immediately with "recommendation run already in progress". Verified live (parallel POSTs + the scheduler's own startup job deflected mid-run). Date-dependent test fixed: FUTTIES active from 07-18 broke an exact-count assertion; `_calendar_context` now patched in that test.
- [x] **Session 40:** Real browser test harness (`tests/browser/test_ui_real.py`, Playwright over CDP to live Electron) — 10/10 views working, 0 console/network errors; found the JS Rice/"price" name-collision, missing search empty-state, blank stats values, stale fodder 81.
- [x] **Session 41:** Word-boundary name matching in all three match paths (signal_tagger fuzzy-confirm gate, context_builder._match_cards, main.cjs buildAskContext, accent-insensitive); purged 146 false-positive tags (Rice 74/85 = 87.1%, Son 72/84 = 85.7%); stats bar COALESCE fix (NULL SUM over empty outcomes JOIN, not case sensitivity); fodder sweep range 82–91 → 81–93 (81/92/93 had been frozen since April 26; verified fresh post-fix).
